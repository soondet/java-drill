...
(async () => {
     ...
     значении: хуже стать не может, а опускается он по мере переписывания. */
  {
    ...
    /* ---- подсказка формулировкой ----
       Длину выровняли — осталась интонация. В ревью блокирующее замечание автор пишет
       утверждением, а вкусовщину и «верно, но не срочно» — советом: «стоит», «лучше», «надо».
       Вычеркнуть варианты с этими словами и ткнуть в остаток наугад давало 39% против 25%,
       в английском should не стоял ни в одном верном из сорока при 43 неверных. На настоящем
       ревью блокирующее пишут так же вежливо, так что перекос — привычка автора, а не жанр.
       Меряем обе тактики: «вычеркни помеченные» и «жми помеченный». */
    const вежл = JSON.parse(await A.ev(`(function(){
      var R=window.REVIEW||[], en=function(s){ return trLook(String(s)); };
      var мера=function(Q,re){ var выч=0, жми=0, уВ=0, уН=0;
        Q.forEach(function(q){ var o=[q.c].concat(q.w), m=o.map(function(x){ return re.test(x); });
          var с=m.filter(Boolean).length, ост=4-с; if(m[0]) уВ++; уН+=с-(m[0]?1:0);
          выч+= ост===0 ? 0.25 : (m[0] ? 0 : 1/ост);  жми+= с===0 ? 0.25 : (m[0] ? 1/с : 0); });
        return {n:Q.length, выч:Math.round(выч/Q.length*100), жми:Math.round(жми/Q.length*100), уВ:уВ, уН:уН}; };
      var ru=R.map(function(r){ return {c:r.correct, w:r.wrong.slice(0,3)}; });
      var e=R.map(function(r){ var c=en(r.correct), w=r.wrong.slice(0,3).map(en); return (c&&w.every(Boolean))?{c:c,w:w}:null; }).filter(Boolean);
      return JSON.stringify({ru:мера(ru,/лучше|стоит|надо/i), en:мера(e,/\\b(should|better|worth)\\b/i)});
    })()`));
    const ПОТОЛОК_СЛОВ = 32, худшее = Math.max(вежл.ru.выч, вежл.ru.жми, вежл.en.выч, вежл.en.жми);
    check("ревью не пройти по вежливым словам", вежл.ru.n >= 40 && вежл.en.n === вежл.ru.n && худшее <= ПОТОЛОК_СЛОВ,
      ["ru", "en"].map(я => я + ": вычеркни " + вежл[я].выч + "% · жми " + вежл[я].жми + "% (слово у " + вежл[я].уВ + " верных и " + вежл[я].уН + " неверных)").join(" · ")
        + " — потолок " + ПОТОЛОК_СЛОВ + "%, наугад 25%");
  }
  ...
  /* ---- подписи интерфейса: у каждого tr("…") есть перевод ----
     Проверки ниже меряют долю кириллицы на вкладках и пропускают всё, что живёт
     внутри раундов и экранов итога. Так в английском ревью вопрос, вердикт и кнопка
     «Дальше →» остались русскими, а по всему приложению без перевода было 117 подписей
     из 675. Ключи берутся из исходника, спрашивается сам словарь страницы — тем же
     trLook, что рисует интерфейс, поэтому «есть в файле, но не доехало» тоже видно. */
  {
    const src = fs.readFileSync(path.join(__dirname, "index.html"), "utf8"), ключи = new Set();
    for (const m of src.matchAll(/\btr\((?:"((?:[^"\\]|\\.)*)"|'((?:[^'\\]|\\.)*)')\)/g)) {
      let k; try { k = JSON.parse('"' + (m[1] != null ? m[1] : m[2].replace(/"/g, '\\"')) + '"'); } catch (e) { continue; }
      if (/[А-Яа-яЁё]/.test(k)) ключи.add(k);
    }
    const немые = JSON.parse(await A.ev(`JSON.stringify(${JSON.stringify([...ключи])}.filter(function(k){ return trLook(k)==null; }))`));
    check("у каждой подписи tr(\"…\") есть перевод", ключи.size > 500 && немые.length === 0,
      немые.length ? немые.length + " из " + ключи.size + " без перевода · например " + немые.slice(0, 4).map(k => "«" + k.slice(0, 40) + "»").join(", ")
                   : ключи.size + " подписей, все в словаре");
  }
